module.exports = {
  apps: [
    {
      name: 'digihip-dev',
      script: 'npm',
      args: 'run dev',
      cwd: '/home/csd3927/digihip',
      env: {
        NODE_ENV: 'development',
        PORT: 3001,
        MONGODB_URI: 'mongodb+srv://giorgosgts1999:3efKH74k1jUG3rkI@cluster0.b3pvc.mongodb.net/digihip?retryWrites=true&w=majority&appName=Cluster0',
        JWT_SECRET: 'nA7<$/%@n7r9of|F3z=@sO^;S>ZH7z3m\'%upW,TPG3(pd[<U-"f<TK)yNH',
        NEXT_PUBLIC_API_URL: 'http://139.91.210.34:3001',
        FIREBASE_PROJECT_ID: 'digiheap-c4aed',
        FIREBASE_CLIENT_EMAIL: 'firebase-adminsdk-fbsvc@digiheap-c4aed.iam.gserviceaccount.com',
        FIREBASE_PRIVATE_KEY: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
      },
    },
  ],
};
